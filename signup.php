      ...
      // Vérification du formulaire d'inscription ------------------------------------------------
          ...
          if (isset($_POST['signup-submit'])) {
            ...
            if (empty($prenom) || empty($nom) || empty($pseudo) || empty($mdp) || empty($cmdp)) {
              ...
              $stmt = mysqli_stmt_init($mysqli); //Je vais préférer une requête préparée
              mysqli_stmt_prepare($stmt, $sql);
              mysqli_stmt_bind_param($stmt, "s", $pseudo);//"s" pour dire que je cherche une string
              ...
              if ($resultChek > 0) {
                ...
                $sql2 = "INSERT INTO membres (gravatar, login, password, statut, prenom, nom) VALUES ('$fileNameNew', '$pseudo', '$hashedmdp', 'membre', '$prenom', '$nom')";
                $query = mysqli_query($conn, $sql2);
                if ($query == true) {
                  echo "<div style='text-align:center' class='alert alert-success' role='alert'>Votre compte à été créé</div><br>";
          ...
          <div class="input-group">
            ...
            <br><br><br>
          <button type="submit" name="signup-submit" class="btn offset-5 button">S'inscrire</button>
        </form>
      </div>
    </div>
<?php
